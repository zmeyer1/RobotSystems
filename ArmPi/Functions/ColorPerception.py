...
class ColorTracking:
    ...
    def find_target_color(self, img: np.array) -> np.array:
        ...
        for i in color_range:
            if i in self.target_colors:
                self.detect_color = i
                frame_mask = cv2.inRange(frame_lab, color_range[self.detect_color][0], color_range[self.detect_color][1])  # Bitwise operation on the original image and mask
                ...
                if area_max > 2500:  # Found the largest area
                    ...
                    img_centerx, img_centery = getCenter(self.rect, self.roi, self.resized_shape, square_length)  # Get the center coordinates of the block
                    ...
                    cv2.putText(img, '(' + str(self.world_x) + ',' + str(self.world_y) + ')', (min(box[0, 0], box[2, 0]), box[2, 1] - 10),
                            cv2.FONT_HERSHEY_SIMPLEX, 0.5, range_rgb[self.detect_color], 1) # Draw the center point
    ...
    def find_persistent_location(self, img: np.array) -> np.array:
        ...
        if self.distance < 0.5:
            ...
            elif time.time() - self.persistant_start > 0.5:
