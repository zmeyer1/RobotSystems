...
# Parameter is the list of contours to compare
...
# Angle of the gripper when grasping
servo1 = 500
...
# Set the RGB light color of the expansion board to be consistent with the color to be tracked
def set_rgb(color):
    if color == "red":
...
def move():
    ...
    # Different color blocks placement coordinates (x, y, z)
    coordinate = {
    ...
    while True:
        if __isRunning:
            if first_move and start_pick_up: # When the object is detected for the first time               
                ...
                setBuzzer(0.1)               
                result = AK.setPitchRangeMoving((world_X, world_Y - 2, 5), -90, -90, 0) # Do not fill in the runtime parameter, adapt the runtime
                if not result:
                    unreachable = True
                else:
                    unreachable = False
                time.sleep(result[2]/1000) # The third item in the return parameter is time
                start_pick_up = False
                first_move = False
                action_finish = True
            elif not first_move and not unreachable: # Not the first time detecting the object
                set_rgb(detect_color)
                if track: # If in tracking phase
                    ...
                    time.sleep(0.02)                    
                    track = False
                if start_pick_up: # If the object hasn't moved for a while, start grasping
                    action_finish = False
                    if not __isRunning: # Stop and exit flag detection
                        continue
                    Board.setBusServoPulse(1, servo1 - 280, 500)  # Gripper opens
                    ...
                    if not __isRunning:
                        continue
                    Board.setBusServoPulse(1, servo1, 500)  # Gripper closes
                    time.sleep(1)
                    ...
                    if not __isRunning:
                        continue
                    Board.setBusServoPulse(1, servo1 - 200, 500)  # Gripper opens, place the object down
                    time.sleep(0.8)
...
def run(img):
    ...
    # If an object is detected in a region, continue to detect that region until it's gone
    ...
    if not start_pick_up:
        ...
        if area_max > 2500:  # Found the largest area
            ...
            distance = math.sqrt(pow(world_x - last_x, 2) + pow(world_y - last_y, 2)) # Compare with the last coordinates to determine whether it moved
